...
/**
 * list uid,pid of pi_flexform, that match specific criteria
 * i.e. table=tt_content list_type=tool_pi2 key=settings.kursbeginnClubEinblenden
 */
class FlexFormsCommand extends Command
{
    protected static $defaultName = 'migrate:flexforms';  //To make your command lazily loaded
    ...
    {
        $this
            ->setDescription('list uid,pid of pi_flexform, that match specific criteria.')
            ->setHelp('i.e. table=tt_content list_type=tool_pi2 key=settings.kursbeginnClubEinblenden. To get more detailed information, use the --verbose option.')
            ->addOption(
                'pid',
                'p',
            ...
            ->addOption(
                ...
                'If this option is set, use given table for research.'
            )
            ->addOption(
                'list_type',
                'l',
                InputOption::VALUE_NONE,
                'If this option is set, focus on given list_type only.'
            )
            ->addOption(
                'key',
                'k',
                InputOption::VALUE_NONE,
                'If this option is set, filter results on flexforms that contain a given key value.'
            )
            ->addOption(
                'dry-run',
                null,
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $table = ($input->hasOption('table')) ? $input->getOption('table') : 'tt_content';
        $list_type = ($input->hasOption('list_type')) ? $input->getOption('list_type') : 'tool_pi2';
        $key = ($input->hasOption('key')) ? $input->getOption('key') : 'settings.kursbeginnClubEinblenden';

        if ($io->isVerbose()) {
            $io->section('Searching the database now for records with FlexForms that match the given criteria.');
        }
     ...
     * @return array
     */
    protected function findAllFlexformsInPage(int $pageId, int $depth, array $dirtyFlexFormFields = []): array
    {
        if ($pageId > 0) {
